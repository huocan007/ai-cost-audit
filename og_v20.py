"""OG Image v20 — Gateway gets 'Smart Router' subtitle."""
...
sub_node_font = ImageFont.truetype(FONT_REG, 11)
...
# Gateway: title + Smart Router subtitle
gx, gy = 368, 290
centered_text(draw, gx, gy, "Gateway", node_font, WHITE)
# subtitle below, offset ~16px down from center
bbox = draw.textbbox((0, 0), "Gateway", font=node_font)
gh = bbox[3] - bbox[1]
draw.text((gx, gy + gh // 2 + 2), "Smart Router",
          font=sub_node_font, fill=WHITE, anchor="mm",
          options=None) if False else None
# simpler: compute subtitle y and center it
sbbox = draw.textbbox((0, 0), "Smart Router", font=sub_node_font)
stw, sth = sbbox[2] - sbbox[0], sbbox[3] - sbbox[1]
draw.text((gx - stw // 2, gy + gh // 2 + 3), "Smart Router",
          font=sub_node_font, fill=WHITE)
